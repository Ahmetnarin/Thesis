...
\newpage
	...
	\begin{description}
		...
		Die Verwendung der nichtlineare Funktionen nach jeder convolutional layer verbessert die Trainingseffizienz im Vergleich zur traditionell verwendeten $\tanh$-Funktionen und gewann im Jahr 2012 ImageNet Large-Scale Visual Recognition Challenge (ILSVRC) mit großem Vorsprung.
	...
	Aufbauend auf diese einfaches Modul wurden mehrere verbesserte und erweiterte Versionen von GoogleNet vorgestellt. \cite{GoingDeeperwithConvolutions}
...
\end{figure}
	...
	In diesem Projekt wurde ein systolisches Array zur Berechnung der Matrixmultiplikation entworfen und implementiert. Die Vorteile des systolisches Arrays sind einfaches Design und zeitgleiche Operationen (Parallelität). Die Nachteile sind, dass es nicht für allgemeine Anwendungen anwendbar ist, sondern eher für speziale Zwecke geeignet ist, da die Organisation von PE (Processing element) zur Berechnung der Matrixelementen anpassen muss. Außerdem die Implementierung des systolichen Arrays ist beschränkt auf 3x3-Matrizen und die Dimension der Matrizen war nicht dynamisch, dass man am Anfang der Berechnung beliebig groß wählen darf. 
    ...
    In meiner Thesis werden zuerst die Grundlagen zum maschinellen Lernen erläuert und im folgenden dann die Strukturen der künstlichen neuronalen Netze betrachtet. Besonders wird die CNN-Architektur \textit{(convolutional neural network)} und ihr Entwicklungstrend betrachtet. Die Gemeinsamkeit von dieser vorgestellten Architekturen sind, dass die erste Entwurf wie z.B. LeNET waren oder die besondere Erfolge in ImageNet Large-Scale Visual Recognition Challenge (ILSVRC) bekommen haben. siehe \autoref{fig:GoogleNet} \autoref{fig:alexnet}
